...
let unfinished = data.filter(function(current) {
  return current["completed"] == false;
});
//console.log(unfinished);
// filter for user 6's tasks
let userSix = data.filter(function(current) {
  return current["userId"] == 6;
});
//console.log(userSix);
// filter for tasks with a title less than 24 characters long
let tfChar = data.filter(function(current) {
  return current["title"].length < 24;
});
console.log(tfChar);
